...
* The analog sensor top have been forfeited: no tampering detection (but simpler architecture).
...
### Cluster
It is possible to offload to cluster the execution of some tasks. OpenTitan can set/unset the fetch enable of the cluster, and is capable of polling a EOC register connected to the corresponding signal of the cluster. The testbench is configured to automatially preload via JTAG the cluster binary (including eventual sections within the L1 as OpenTitan has access to the L1 and cluster control unit) when specified by "cl-bin" argument. The fetch enable and EOC registers are mapped as follows:
|   | Address  | ResVal  |
|---|---|---|
