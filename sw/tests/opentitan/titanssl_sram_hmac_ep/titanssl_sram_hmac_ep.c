...
#define TITANSSL_SIZE_PAYLOAD 64    
        ...
        char **argv)
{
    ...
    *tmp = 1;
    ...
    start = (uint64_t*) 0x80700000;
    end = (uint64_t*) 0x80700010;
    tot = (uint64_t*) 0x80700020;
    ...
    *tot = (*end) - (*start);
    printf("cycles start: %u end: %u tot: %u \r\n", *start, *end, *tot);
    // printf("cycles start: %u end: %u tot: %u \r\n", *start, *end, *tot);

#if TITANSSL_CFG_DEBUG
	for (int i=0; i<(buffer_digest.n)/4; i++)
	{
		printf("0x%08x\r\n", ((uint32_t*)buffer_digest.data)[i]);
        uart_wait_tx_done();
	}
#endif
